...
STATEFULSET_NAME="${RELEASE_NAME:-ongdb}-ongdb"
ONGDB_SECRETS_PASSWORD=$(kubectl get secret -n ${NS} ${RELEASE_NAME}-ongdb-secrets -o jsonpath='{.data.ongdb-password}' | base64 --decode)
...
check_role() {
  ...
  while true; do
    ...
    kubectl exec ${name} -n ${NS} -- bin/geequel-shell -u ongdb -p ${ONGDB_SECRETS_PASSWORD} "call dbms.cluster.role()" 2>/dev/null
    response_code=$?
